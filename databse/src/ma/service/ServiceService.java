...
public class ServiceService implements IDao<Service>{
    ...
    @Override
    public boolean update(Service o, Service p) {
        try {
            String req = "Update service set nom = ? where id = "+o.getId();
            PreparedStatement ps = Connexion.getConnection().prepareStatement(req);
            ps.setString(1, p.getNom());
            if(ps.executeUpdate() == 1)
                return true;
        } catch (SQLException ex) {
            System.out.println("Erreur de modification du service:" +ex.getMessage());
        }
        return false;
    }
